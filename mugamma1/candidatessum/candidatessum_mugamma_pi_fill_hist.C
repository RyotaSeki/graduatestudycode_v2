void candidatessum_mugamma_pi_fill_hist()  //(const char* filename)
{
  ...
  std::string filename = Form("Candidates mugamma1 pi Sum");
  std::string cname = "c_" + filename;
  TCanvas* c = new TCanvas(cname.c_str(), cname.c_str(), 840, 560);
  TH1* h = new TH1F(("h_"+filename).c_str(), ";Candidates;Entryies of mugamma1 pi ", 200, 0, 8);
