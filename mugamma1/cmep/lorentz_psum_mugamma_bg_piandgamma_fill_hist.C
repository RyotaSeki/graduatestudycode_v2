...
void lorentz_psum_mugamma_bg_piandgamma_fill_hist()  //(const char* filename)
{
  ...
  Int_t t = 29;
  Int_t i;
  //t_pi->Add("/home/usr/data/belle2/mc/BGx1/mugamma1_0010_BGx1_pi.root");
  ...
  std::string filename = Form("CM_Psum_mugamma_bg pi and gammma");
  std::string cname = "c_" + filename;
  TCanvas* c = new TCanvas(cname.c_str(), cname.c_str(), 840, 560);
  TH1* h = new TH1F(("h_"+filename).c_str(), ";CM momentum[GeV]; ", 300, 0, 15);  
   		...
   		if(i_pi_evtNum  == i_gamma_evtNum){
			Esum = 0; pxsum = 0; pysum = 0; pzsum = 0; 
    			...
    			for (int in = 0; in < gamma_ncandidates; in++) {
    				...
    				TLorentzVector p4 = Lorentz(p,gamma_E[in]);	
			...
			num++;
			float psum = pxsum*pxsum+pysum*pysum+pzsum*pzsum;	psum = sqrtf(psum);
			if(pi_ncandidates!=0 && gamma_ncandidates!=0)h->Fill(psum);
    			il++;
			break;
		}
    	}
