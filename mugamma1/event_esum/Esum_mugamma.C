...
void Esum_mugamma()  //(const char* filename)
{
  ...
  std::string filename = Form("Esum mugamma1 pi and gammma");
  std::string cname = filename;
  TCanvas* c = new TCanvas(cname.c_str(), cname.c_str(), 840, 560);
  TH1* h = new TH1F((filename).c_str(), ";Energy[GeV];Entry of pi and gamma Sum  ", 300, 0, 11);  
