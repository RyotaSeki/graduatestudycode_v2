...
void cos(const std::string filename)  //(const char* filename)
...
*/  
  ...
  */
  ...
  TH1* h = new TH1F(("h_"+filename).c_str(), ";cos", 300, -1, 1);	//ヒストグラムの範囲
    ...
    for (int i = 0; i < t_pi -> GetEntries(); i++) {
        ...
        for(il ; il < t_gamma -> GetEntries();il++){
            ...
            if(pi_evtNum  == gamma_evtNum){
                ...
                //if(v.Mag() < 0.80 ||v.Mag() > 0.96)break;
                for (int in = 0; in < pi_ncandidates; in++) {
                    ...
                    for (int im = 0; im < gamma_ncandidates; im++){
                        ...
                        /*if(minv >=1.65 && minv <= 1.85 && deltae >= -0.4 && deltae <= 0.1){
                            h->Fill(minv,deltae);
                            select ++;
 ...
 h->Draw("box");
 //h->Draw("hist");
 c1->SaveAs(Form("./cospng/%s.png",filename.c_str()));
