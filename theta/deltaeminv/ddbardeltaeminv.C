...
void ddbardeltaeminv()  //(const char* filename)
...
*/
  ...
  Int_t i;
  for(i=11;i<=t;i++){
  	t_pi->Add(Form("/home/usr/data/belle2/mc/BGx1/ddbar_BGx1_00%d_pi.root",i));
  }
  TChain *t_gamma=new TChain("t_gamma","");
  for(i=11;i<=t;i++){
  	t_gamma->Add(Form("/home/usr/data/belle2/mc/BGx1/ddbar_BGx1_00%d_gamma.root",i));
  }
  
  
  //std::string filename = Form("ccbar0010");
  std::string filename = Form("ddbar");
