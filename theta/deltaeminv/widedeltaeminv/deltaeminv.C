...
void deltaeminv(const std::string filename)  //(const char* filename)
...
*/  
  ...
  */
  ...
  TH2* h = new TH2F(("h_"+filename).c_str(), ";Minv[GeV/c^2];DeltaE[GeV]", 50, 1.5, 1.9,50,-0.6,0.4);	//ヒストグラムの範囲
            ...
            if(pi_evtNum  == gamma_evtNum){
                ...
                for (int in = 0; in < pi_ncandidates; in++) {
                    ...
                    for (int im = 0; im < gamma_ncandidates; im++){
                        ...
                        if(minv >=1.5 && minv <= 1.9 && deltae >= -0.6 && deltae <= 0.4){
                            h->Fill(minv,deltae);
                            select ++;
