...
void mugammadeltaeminv()  //(const char* filename)
...
*/
  ...
  //std::string filename = Form("ssbar0011");
  ...
  gStyle->SetStatX(0.4);						//canvasのx位置
  ...
  */
  ...
  TH2* h = new TH2F(("h_"+filename).c_str(), ";Minv[GeV/c^2];DeltaE[GeV]", 50, 1.6, 1.95,50,-0.7,0.4);	//ヒストグラムの範囲
    ...
    for (int i = 0; i < t_pi -> GetEntries(); i++) {
        ...
        for(il ; il < t_gamma -> GetEntries();il++){
            ...
            if(pi_evtNum  == gamma_evtNum){
                ...
                if(v.Mag() < 0.90 ||v.Mag() > 0.96)break;
                for (int in = 0; in < pi_ncandidates; in++) {
                    ...
                    for (int im = 0; im < gamma_ncandidates; im++){
                        ...
                        if(minv >=1.6 && minv <= 1.95 && deltae >= -0.7 && deltae <= 0.4){
                            h->Fill(minv,deltae);
                            select ++;
                        }
 ...
 std::cout << filename << "entries" <<  t_pi->GetEntries() << std::endl;
 //scanf_s()
 //h1->Draw("same");
 //h2->Draw("same");
 //h3->Draw("same");
