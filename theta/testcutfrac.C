...
*/  
  ...
  */
  ...
  for(float cut = 0; cut < 100; cut+=1)
  {
            ...
            if(pi_evtNum  == gamma_evtNum){
                ...
                for (int in = 0; in < pi_ncandidates; in++) {
                    ...
                    for (int im = 0; im < gamma_ncandidates; im++){
                        ...
                        if(deltae=>-5&&deltae=<5&&minv=>0&&minv=<3)cutnum++
    ...
    x[cut]=cut/100;
    y[cut]=cutnum/cutsum;
    cutnum=0;cutsum=0;cutfrac=0;
}
 TGraph* t = new TGraph(100, x, y);
 //h->Draw("hist");
 c1->SaveAs(Form("./trash/%s.png",filename.c_str()));
